...
! BEM---------------------------------------------
! Add ORG as output variables BEM 2010
    call addfld ('ORG',     'unitless',       1, 'A', 'Convective Organization', phys_decomp)           !BEM 2010 
    call addfld ('ORG_ADV', 'unitless',    pver, 'A', 'Convective Organization- Advected', phys_decomp) !BEM 2010
    call addfld( 'Cu_evapprec', 'kg/m2/s'   , 1 , 'A', 'Integrated precip evaporation'    , phys_decomp )!BEM 2010
    call addfld( 'Cu_evaprain', 'kg/m2/s'   , 1 , 'A', 'Integrated rain   evaporation'    , phys_decomp )!BEM 2010

    ! ------------------------- !
    ! Internal Output Variables !
    ! ------------------------- !

    call addfld( 'Cu_qtflx'       , 'kg/m2/s' , pverp , 'A' , 'Convective qt flux'                                  , phys_decomp )
    call addfld( 'Cu_slflx'       , 'J/m2/s'  , pverp , 'A' , 'Convective sl flux'                                  , phys_decomp )
    call addfld( 'Cu_uflx'        , 'kg/m/s2' , pverp , 'A' , 'Convective  u flux'                                  , phys_decomp )
    call addfld( 'Cu_vflx'        , 'kg/m/s2' , pverp , 'A' , 'Convective  v flux'                                  , phys_decomp )

    call addfld( 'Cu_qtten'       , 'kg/kg/s' , pver  , 'A' , 'qt tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_slten'       , 'J/kg/s'  , pver  , 'A' , 'sl tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_uten'        , 'm/s2'    , pver  , 'A' , ' u tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_vten'        , 'm/s2'    , pver  , 'A' , ' v tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_qvten'       , 'kg/kg/s' , pver  , 'A' , 'qv tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_qlten'       , 'kg/kg/s' , pver  , 'A' , 'ql tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_qiten'       , 'kg/kg/s' , pver  , 'A' , 'qi tendency by convection'                           , phys_decomp )
    call addfld( 'Cu_qcten'       , 'kg/kg/s' , pver  , 'A' , '(qi+ql) tendency by convection'                      , phys_decomp )

    call addfld( 'Cu_cbmf'        , 'kg/m2/s' , 1     , 'A' , 'Cumulus base mass flux'                              , phys_decomp )
    call addfld( 'Cu_ufrcinvbase' , 'fraction', 1     , 'A' , 'cu fraction at PBL top'                              , phys_decomp ) 
    call addfld( 'Cu_ufrclcl'     , 'fraction', 1     , 'A' , 'Cumulus fraction at LCL'                             , phys_decomp )
    call addfld( 'Cu_winvbase'    , 'm/s'     , 1     , 'A' , 'Cumulus vertical velocity at PBL top'                , phys_decomp )
    call addfld( 'Cu_wlcl'        , 'm/s'     , 1     , 'A' , 'Cumulus vertical velocity at LCL'                    , phys_decomp )
    call addfld( 'Cu_plcl'        , 'Pa'      , 1     , 'A' , 'LCL of source air'                                   , phys_decomp )
    call addfld( 'Cu_pinv'        , 'Pa'      , 1     , 'A' , 'PBL top pressure'                                    , phys_decomp )
    call addfld( 'Cu_plfc'        , 'Pa'      , 1     , 'A' , 'LFC of source air'                                   , phys_decomp )
    call addfld( 'Cu_pbup'        , 'Pa'      , 1     , 'A' , 'Highest interface level of positive cumulus buoyancy', phys_decomp )
    call addfld( 'Cu_ppen'        , 'Pa'      , 1     , 'A' , 'Highest level where cumulus w is 0'                  , phys_decomp )
    call addfld( 'Cu_srcqt'       , 'kg/kg'   , 1     , 'A' , 'Cumulus source air qt'                               , phys_decomp )
    call addfld( 'Cu_srcthl'      , 'K'       , 1     , 'A' , 'Cumulus source air thl'                              , phys_decomp )
    call addfld( 'Cu_srcthvl'     , 'K'       , 1     , 'A' , 'Cumulus source air thvl'                             , phys_decomp )
    call addfld( 'Cu_emfkbup'     , 'kg/m2/s' , 1     , 'A' , 'Penetrative mass flux at kbup'                       , phys_decomp )
    call addfld( 'Cu_cin'         , 'J/kg'    , 1     , 'A' , 'CIN upto LFC'                                        , phys_decomp )
    call addfld( 'Cu_cinlcl'      , 'J/kg'    , 1     , 'A' , 'CIN upto LCL'                                        , phys_decomp )
    call addfld( 'Cu_cbmflimit'   , 'kg/m2/s' , 1     , 'A' , 'cbmflimiter'                                         , phys_decomp ) 
    call addfld( 'Cu_tkeavg'      , 'm2/s2'   , 1     , 'A' , 'Average tke within PBL for convection scheme'        , phys_decomp ) 
    call addfld( 'Cu_zinv'        , 'm'       , 1     , 'A' , 'PBL top height'                                      , phys_decomp )
    call addfld( 'Cu_rcwp'        , 'kg/m2'   , 1     , 'A' , 'cu LWP+IWP'                                          , phys_decomp ) !BEMens 2010
    call addfld( 'Cu_rlwp'        , 'kg/m2'   , 1     , 'A' , 'cu LWP'                                              , phys_decomp ) !BEMens
    call addfld( 'Cu_riwp'        , 'kg/m2'   , 1     , 'A' , 'cu IWP'                                              , phys_decomp ) !BEMens
    call addfld( 'Cu_tophgt'      , 'm'       , 1     , 'A' , 'Cumulus top height'                                  , phys_decomp )

    call addfld( 'Cu_wu'          , 'm/s'     , pverp , 'A' , 'Convective updraft vertical velocity'                , phys_decomp )
    call addfld( 'Cu_ufrc'        , 'fraction', pverp , 'A' , 'Convective updraft fractional area'                  , phys_decomp )
    call addfld( 'Cu_qtu'         , 'kg/kg'   , pverp , 'A' , 'Cumulus updraft qt'                                  , phys_decomp )
    call addfld( 'Cu_thlu'        , 'K'       , pverp , 'A' , 'Cumulus updraft thl'                                 , phys_decomp )
    call addfld( 'Cu_thvu'        , 'K'       , pverp , 'A' , 'Cumulus updraft thv'                                 , phys_decomp )
    call addfld( 'Cu_uu'          , 'm/s'     , pverp , 'A' , 'Cumulus updraft uwnd'                                , phys_decomp )
    call addfld( 'Cu_vu'          , 'm/s'     , pverp , 'A' , 'Cumulus updraft vwnd'                                , phys_decomp )
    call addfld( 'Cu_qtu_emf'     , 'kg/kg'   , pverp , 'A' , 'qt of penatratively entrained air'                   , phys_decomp )
    call addfld( 'Cu_thlu_emf'    , 'K'       , pverp , 'A' , 'thl of penatratively entrained air'                  , phys_decomp )
    call addfld( 'Cu_uu_emf'      , 'm/s'     , pverp , 'A' , 'uwnd of penatratively entrained air'                 , phys_decomp )
    call addfld( 'Cu_vu_emf'      , 'm/s'     , pverp , 'A' , 'vwnd of penatratively entrained air'                 , phys_decomp )
    call addfld( 'Cu_umf'         , 'kg/m2/s' , pverp , 'A' , 'Cumulus updraft mass flux'                           , phys_decomp )
    call addfld( 'Cu_uemf'        , 'kg/m2/s' , pverp , 'A' , 'Cumulus net ( updraft + entrainment ) mass flux'     , phys_decomp )
    call addfld( 'Cu_qcu'         , 'kg/kg'   , pver  , 'A' , 'Cumulus updraft LWC+IWC'                             , phys_decomp )
    call addfld( 'Cu_qlu'         , 'kg/kg'   , pver  , 'A' , 'Cumulus updraft LWC'                                 , phys_decomp )
    call addfld( 'Cu_qiu'         , 'kg/kg'   , pver  , 'A' , 'Cumulus updraft IWC'                                 , phys_decomp )
    call addfld( 'Cu_cufrc'       , 'fraction', pver  , 'A' , 'Cumulus cloud fraction'                              , phys_decomp )
    call addfld( 'Cu_fer'         , '1/Pa'     , pver  , 'A' , 'Cumulus lateral fractional entrainment rate'         , phys_decomp )
    call addfld( 'Cu_fdr'         , '1/Pa'     , pver  , 'A' , 'Cumulus lateral fractional detrainment Rate'         , phys_decomp )

!BEM renamed these for alphabetical order and clarity
    call addfld( 'Cu_preover'       , 'kg/kg/s' , pver  , 'A' , 'Expulsion rate of cumulus condensate to env.'       , phys_decomp )
    call addfld( 'Cu_priover'       , 'kg/kg/s' , pver  , 'A' , 'Expulsion rate of cumulus ice water to env.'        , phys_decomp )
    call addfld( 'Cu_preprod'       , 'kg/kg/s' , pver  , 'A' , 'Production rate of precip by cumulus'                , phys_decomp )
    call addfld( 'Cu_priprod'       , 'kg/kg/s' , pver  , 'A' , 'Production rate of snow by cumulus'                  , phys_decomp )
    call addfld( 'Cu_preprodnet'    , 'kg/kg/s' , pver  , 'A' , 'Net production rate of precip by Cumulus'            , phys_decomp )
    call addfld( 'Cu_priprodnet'    , 'kg/kg/s' , pver  , 'A' , 'Net production rate of snow by Cumulus'              , phys_decomp )
    call addfld( 'Cu_preqflx'        , 'kg/m2/s' , pverp , 'A' , 'Precip flux induced by Cumulus'                      , phys_decomp )
    call addfld( 'Cu_priqflx'        , 'kg/m2/s' , pverp , 'A' , 'Snow flux induced by Cumulus'                        , phys_decomp )

!BEM added these 2 more for alphabetical order and clarity: layers not interfaces
    call addfld( 'Cu_prerevap'      , 'kg/m2/s' , pver , 'A' , 'Precip reevap in Cu'                        , phys_decomp )
    call addfld( 'Cu_prirevap'      , 'kg/m2/s' , pver , 'A' , 'Snow reevap in Cu'                          , phys_decomp )

!BEM Horizontal (di)vergence in and out of plumes
    call addfld( 'Cu_vergin'        , '/s'      , pverp , 'A' , 'entrainment massflux/dp in Cu'              , phys_decomp )
    call addfld( 'Cu_vergout'       , '/s'      , pverp , 'A' , 'detrainment massflux/dp in Cu'              , phys_decomp )
    call addfld( 'Cu_vergence'      , '/s'      , pverp , 'A' , 'det-ent massflux/dp in Cu'                  , phys_decomp )
    call addfld( 'Cu_dMdp'          , '/s'      , pverp , 'A' , '-d(umf)/dp in Cu'                           , phys_decomp )

!BEM individual plume fields
    call addfld( 'Cu_plume1_b'         , 'MKS'     , pverp, 'A', 'Updraft buoyancy'                             , phys_decomp )
    call addfld( 'Cu_plume1_w'         , 'MKS'     , pverp, 'A', 'Updraft vertical velocity'                    , phys_decomp )
    call addfld( 'Cu_plume1_Mup'       , 'MKS'     , pverp, 'A', 'Updraft massflux'                             , phys_decomp )
    call addfld( 'Cu_plume1_MF'        , 'MKS'     , pverp, 'A', 'Massflux (updraft plus penetrative)'          , phys_decomp )
    call addfld( 'Cu_plume1_ufrc'      , 'MKS'     , pverp, 'A', 'Fractional area'                              , phys_decomp )

    call addfld( 'Cu_plumeN_b'         , 'MKS'     , pverp, 'A', 'Updraft buoyancy'                             , phys_decomp )
    call addfld( 'Cu_plumeN_w'         , 'MKS'     , pverp, 'A', 'Updraft vertical velocity'                    , phys_decomp )
    call addfld( 'Cu_plumeN_Mup'       , 'MKS'     , pverp, 'A', 'Updraft massflux'                             , phys_decomp )
    call addfld( 'Cu_plumeN_MF'        , 'MKS'     , pverp, 'A', 'Massflux (updraft plus penetrative)'          , phys_decomp )
    call addfld( 'Cu_plumeN_ufrc'      , 'MKS'     , pverp, 'A', 'Fractional area'                              , phys_decomp )

!BEM block end


    call addfld( 'Cu_excessu'     , 'no'      , pver  , 'A' , 'Updraft saturation excess'                           , phys_decomp )
    call addfld( 'Cu_excess0'     , 'no'      , pver  , 'A' , 'Environmental saturation excess'                     , phys_decomp )
    call addfld( 'Cu_xc'          , 'no'      , pver  , 'A' , 'Critical mixing ratio'                               , phys_decomp )
    call addfld( 'Cu_aquad'       , 'no'      , pver  , 'A' , 'aquad'                                               , phys_decomp )
    call addfld( 'Cu_bquad'       , 'no'      , pver  , 'A' , 'bquad'                                               , phys_decomp )
    call addfld( 'Cu_cquad'       , 'no'      , pver  , 'A' , 'cquad'                                               , phys_decomp )
    call addfld( 'Cu_bogbot'      , 'no'      , pver  , 'A' , 'Cloud buoyancy at the bottom interface'              , phys_decomp )
    call addfld( 'Cu_bogtop'      , 'no'      , pver  , 'A' , 'Cloud buoyancy at the top interface'                 , phys_decomp )

    call addfld('Cu_exit_UWCu'    , 'no'      , 1     , 'A' , 'exit_UWCu'                                           , phys_decomp ) 
    call addfld('Cu_exit_conden'  , 'no'      , 1     , 'A' , 'exit_conden'                                         , phys_decomp ) 
    call addfld('Cu_exit_klclmkx' , 'no'      , 1     , 'A' , 'exit_klclmkx'                                        , phys_decomp ) 
    call addfld('Cu_exit_klfcmkx' , 'no'      , 1     , 'A' , 'exit_klfcmkx'                                        , phys_decomp ) 
    call addfld('Cu_exit_ufrc'    , 'no'      , 1     , 'A' , 'exit_ufrc'                                           , phys_decomp ) 
    call addfld('Cu_exit_wtw'     , 'no'      , 1     , 'A' , 'exit_wtw'                                            , phys_decomp ) 
    call addfld('Cu_exit_drycore' , 'no'      , 1     , 'A' , 'exit_drycore'                                        , phys_decomp ) 
    call addfld('Cu_exit_wu'      , 'no'      , 1     , 'A' , 'exit_wu'                                             , phys_decomp ) 
    call addfld('Cu_exit_cufilter', 'no'      , 1     , 'A' , 'exit_cufilter'                                       , phys_decomp ) 
    call addfld('Cu_exit_kinv1'   , 'no'      , 1     , 'A' , 'exit_kinv1'                                          , phys_decomp ) 
    call addfld('Cu_exit_rei'     , 'no'      , 1     , 'A' , 'exit_rei'                                            , phys_decomp ) 

    call addfld('Cu_limit_shcu'   , 'no'      , 1     , 'A' , 'limit_shcu'                                          , phys_decomp ) 
    call addfld('Cu_limit_negcon' , 'no'      , 1     , 'A' , 'limit_negcon'                                        , phys_decomp ) 
    call addfld('Cu_limit_ufrc'   , 'no'      , 1     , 'A' , 'limit_ufrc'                                          , phys_decomp ) 
    call addfld('Cu_limit_ppen'   , 'no'      , 1     , 'A' , 'limit_ppen'                                          , phys_decomp ) 
    call addfld('Cu_limit_emf'    , 'no'      , 1     , 'A' , 'limit_emf'                                           , phys_decomp ) 
    call addfld('Cu_limit_cinlcl' , 'no'      , 1     , 'A' , 'limit_cinlcl'                                        , phys_decomp ) 
    call addfld('Cu_limit_cin'    , 'no'      , 1     , 'A' , 'limit_cin'                                           , phys_decomp ) 
    call addfld('Cu_limit_cbmf'   , 'no'      , 1     , 'A' , 'limit_cbmf'                                          , phys_decomp ) 
    call addfld('Cu_limit_rei'    , 'no'      , 1     , 'A' , 'limit_rei'                                           , phys_decomp ) 
    call addfld('Cu_ind_delcin'   , 'no'      , 1     , 'A' , 'ind_delcin'                                          , phys_decomp ) 
  ...
  subroutine compute_uwshcu_inv( mix      , mkx        , iend          , ncnst     , dt       ,  & 
                                 ...
                                 landfrac , lon        , lat) !BEM 2011 added landfrac,BHC added lon, lat
    implicit none
    ...
    real(r8), intent(in)    :: lon(mix)                 !BHC2011
    real(r8), intent(in)    :: lat(mix)                 !BHC2011 
    ...
    real(r8), intent(in)    :: landfrac(mix)          ! Land fraction BEM 2011
    ...
    call compute_uwshcu( mix  , mkx    , iend      , ncnst , dt   , &
                         ...
                         landfrac      , lon       , lat ) !BEM 2011 added landfrac
  ...
  subroutine compute_uwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
    ...
    use time_manager,    only : get_curr_date        !BHC2011
    use cam_history,     only : outfld, addfld, phys_decomp
...
!-------------------------------------------------------- BEM 2010 organized ensemble
! BEM 2010 add my declarations right at top for convenience in editor window

    integer iorg                      ! org tracer index !BEM 2010
    real(r8) org(mix), orgsum(mix)   ! BEM 2010  Org, internal 2Dhor variable only
    real(r8) org_clipped(mix)        ! BEM 2010  capped at 1

! ORG is defined functionally, by these coefficients, of two types: 
! what makes org, and what it does. The former are xxx2org, coefficient s
! relating field xxx to an org tendency. The latter org2yyy are coefficients used in 
! corrections yyy to the unorganized (standard) plume in the presence of org. 

    real(r8) tauORG                  ! damping time (s)
    real(r8) evap2org, topo2org, coast2org, rand2org, shear2org  ! source coeffs BEM 2010
    real(r8) org2thpert, org2qpert, org2wpert ! converting org to plume base perts BEM 2010
    real(r8) org2cbmf1, org2cbmf2, org2rkm, org2overlap  ! converting org to closure, entrainment, overlap BEM 2010
    real(r8) org2ensdist              ! how cbmf is distributed over plume set        BEM 2010
 
    parameter (tauORG = 1.e4_r8)      ! 10 ks ~ 3h. Choose these 2 params so          BEM 2010
                                      ! org_steadystate = alf*tau*evpint              BEM 2010
!    parameter (evap2org = 1.e1_r8)    ! is about 1 for evpint ~ 1 mm/d (1e-5 mm/s)   BEM 2010
    parameter (evap2org = 2.0_r8)      ! is about 1 for evpint ~ 1 mm/d (1e-5 mm/s)    BEM 2010
                                      ! experience org~2 peaks to 5 in COARE SCM run  BEM 2010
                                      ! so I reduced this to 2 from 10 so org is in [0,1] mostly
                                      ! that is, 1mm/d reevap --> org of 0.2 or so    BEM 2010
! BEMorg framework for more sources
    parameter (topo2org = 0._r8)      ! subgrid orograpphy as convection organizer    BEM 2010
    parameter (coast2org = 5.e-5_r8)  ! subgrid water source (sea breezes)            BEM 2011
    parameter (shear2org = 0._r8)     ! shear over what layer; what other factors? 0  BEM 2010
    parameter (rand2org = 0._r8)      ! stochastic component strength                 BEM 2010

! BEMorg framework for more org effects: not all active or even built yet
! BEMorg **ORG=1** <--> 5mm/d evap for 3h --> 15K/d over 100mb mean PBL
! BEMorg              --> 2K in 3h. But updraft source is not chilled. 
! BEMorg so let's take org2thpert = 2

    parameter (org2thpert = 0._r8)     ! plume base air differs from area average (K) BEM 2010
    parameter (org2qpert = 0._r8)  ! plume base air differs from average  (kg/kg) BEM 2010
    parameter (org2wpert = 0._r8)      ! plume base air differs from isotropic PBL w' BEM 2010
    parameter (org2cbmf1  = 0._r8)     ! extra mass flux in 1st plume? (w' skew?)     BEM 2010
    parameter (org2cbmf2 = 10._r8)      ! extra mass flux in 2nd member (relative)     BEM 2010
    parameter (org2rkm   = 10._r8)      ! second ens member has rrkm = rkm/(1+this*org)BEM 2010
    parameter (org2overlap= 0._r8)     ! org-> probability of entraining prior plume  BEM 2010

! BEMens declare iens, NENSEMBLE, kpenmax for ensemble of plumes
    integer iens, NENSEMBLE, kpenmax   ! BEMens
    parameter (NENSEMBLE = 2)          ! BEMens
!----------------------------------------------------------------------- BEM 2010
...
!    type(physics_state),   intent(inout)  :: state          !BHC
    ...
    integer , intent(in)    :: iend
    integer , intent(in)    :: ncnst
    integer , intent(out)   :: yr,mon,day,tod      !from time_manager BHC2011
    real(r8), intent(out)   :: GMT                 !GMT from curr_date BHC2011
    real(r8), intent(in)    :: dt                             !  Time step : 2*delta_t [ s ]
    
    real(r8), intent(in)    :: lon(mix)                       !  current longitude  BHC2011
    real(r8), intent(in)    :: lat(mix)                       !  BHC2011
    real(r8), intent(in)    :: LT(mix)                        !  Local time for each column trans from GMT  BHC2011
    ...
    real(r8)                   evapprec_out(mix)               !  Integrated evaporation of precipitation [ kg/m2/s ] !BEM 2010
    real(r8)                   evaprain_out(mix)               !  Integrated evaporation of rain          [ kg/m2/s ] !BEM 2010
...
! variables that need to be carried in an ensemble of plumes !BEMens 2010 
    real(8)   cbmf_enstotal ! BEMens 2010
    real(8)   umf_enstotal(0:mkx) ! BEMens 2010
    real(8)   emf_enstotal(0:mkx) ! BEMens 2010
    real(8)   slflx_enstotal(0:mkx) ! BEMens 2010
    real(8)   qtflx_enstotal(0:mkx) ! BEMens 2010
    real(8)   uflx_enstotal(0:mkx) ! BEMens 2010
    real(8)   vflx_enstotal(0:mkx) ! BEMens 2010
    real(8)   trflx_enstotal(0:mkx,ncnst) ! BEMens 2010
    real(8)   dwten_enstotal(mkx) ! BEMens 2010
    real(8)   diten_enstotal(mkx) ! BEMens 2010
    real(8)   qlten_enstotal(mkx) ! BEMens 2010
    real(8)   qiten_enstotal(mkx) ! BEMens 2010
    real(8)   qc_enstotal(mkx) ! BEMens 2010 
    real(8)   qc_l_enstotal(mkx) ! BEMens 2010 
    real(8)   qc_i_enstotal(mkx) ! BEMens 2010 
    real(8)   ufrc_enstotal(0:mkx) ! BEMens 2010 
    real(8)   cufrc_enstotal(mkx) ! BEMens 2010 
    real(8)   qcu_enssum(mkx) ! BEMens 2010 
    real(8)   qlu_enssum(mkx) ! BEMens 2010 
    real(8)   qiu_enssum(mkx) ! BEMens 2010 
    ...
    real(r8)    rrkm   !BEMens a variable to store the entrainment rate (related to parameter rkm but not equal to it)
...
!BEMens diagnostics from the individual members: 1,2,N for now and just 5 variables
!BEMens why don't these show up in output?? Put here and in namelist. Dang. 

    real(r8)  plume1_b(mix,0:mkx)                            !  Updraft buoyancy
    real(r8)  plume1_w(mix,0:mkx)                            !  Updraft vertical velocity ( defined from the release level to 'kpen-1' interface )
    real(r8)  plume1_Mup(mix,0:mkx)                          !  massflux
    real(r8)  plume1_MF(mix,0:mkx)                           !  massflux (updraft plue penetrative)
    real(r8)  plume1_ufrc(mix,0:mkx)                         !  fractional area

    real(r8)  plumeN_b(mix,0:mkx)                            !  Updraft buoyancy
    real(r8)  plumeN_w(mix,0:mkx)                            !  Updraft vertical velocity ( defined from the release level to 'kpen-1' interface )
    real(r8)  plumeN_Mup(mix,0:mkx)                          !  massflux
    real(r8)  plumeN_MF(mix,0:mkx)                           !  massflux (updraft plue penetrative)
    real(r8)  plumeN_ufrc(mix,0:mkx)                         !  fractional area
!BEMens

!BEM some more diagnostics (not working, grr)
    real(r8)  vergin(mix,0:mkx)                              !  BEM vergence inflow entrained
    real(r8)  vergout(mix,0:mkx)                             !  BEM vergence outflow detrained
    real(r8)  vergence(mix,0:mkx)                            !  BEM vergence (de-en)trained
    real(r8)  dMdp(mix,0:mkx)                                !  BEM vergence from -dM/dp
!BEM these two work though
    real(r8)  qpten_out(mix,mkx)                            ! BEM precip tendentcy [ kg/m2/s ]
    real(r8)  qcten_out(mix,mkx)                            ! BEM cloud tendentcy [ kg/m2/s ]
...
!    qpten_out(:iend,mkx)         = 0.0_r8 : Not used in cesm1_0_3
!    qcten_out(:iend,mkx)         = 0.0_r8

!BEM initialize my new variables
    vergin(:iend,mkx)            = 0.0_r8 !BEM
    vergout(:iend,mkx)           = 0.0_r8
    vergence(:iend,mkx)          = 0.0_r8
    dMdp(:iend,0:mkx)              = 0.0_r8
!BEM individual plume fields
    plume1_b(:,:)                = 0.0_r8
    plume1_w(:,:)                = 0.0_r8
    plume1_Mup(:,:)              = 0.0_r8
...
!      write(iulog,*) 'Longitude in radian =', lon(i)
!      write(iulog,*) 'Latitude in radian =', lat(i)
      id_exit = .false.
...
! BEMens initialize the ensemble totals to zero here at top of code, inside i loop
cbmf_enstotal = 0.0_r8 ! BEMens 2010
umf_enstotal(:) = 0.0_r8 ! BEMens 2010
emf_enstotal(:) = 0.0_r8 ! BEMens 2010
slflx_enstotal(:) = 0.0_r8 ! BEMens 2010
qtflx_enstotal(:) = 0.0_r8 ! BEMens 2010
uflx_enstotal(:) = 0.0_r8 ! BEMens 2010
vflx_enstotal(:) = 0.0_r8 ! BEMens 2010
trflx_enstotal(:,:ncnst) = 0.0_r8 ! BEMens 2010
dwten_enstotal(:) = 0.0_r8 ! BEMens 2010
diten_enstotal(:) = 0.0_r8 ! BEMens 2010
qlten_enstotal(:) = 0.0_r8 ! BEMens 2010
qiten_enstotal(:) = 0.0_r8 ! BEMens 2010 
qc_enstotal(:) = 0.0_r8 ! BEMens 2010 
qc_l_enstotal(:) = 0.0_r8 ! BEMens 2010 
qc_i_enstotal(:) = 0.0_r8 ! BEMens 2010 
cufrc_enstotal(:) = 0.0_r8 ! BEMens 2010 
ufrc_enstotal(:) = 0.0_r8 ! BEMens 2010 
qcu_enssum(:) = 0.0_r8 ! BEMens 2010 
qlu_enssum(:) = 0.0_r8 ! BEMens 2010 
qiu_enssum(:) = 0.0_r8 ! BEMens 2010 
...
!-BEM org After all declarations and initializations -------------------------------
! BEM get index of 3D ORG_ADV variable in "state%q" structure so I can grab it
!    write(iulog,*) 'BEMens DEFINING org sources ', evap2org, topo2org, coast2org, rand2org, shear2org
!    write(iulog,*) 'BEMens DEFINING org impacts ', org2thpert, org2qpert, org2wpert, org2cbmf1, org2cbmf2, org2rkm, org2overlap   

   call cnst_get_ind('ORG_ADV',iorg)       !BEM 2010, from Rich Neale version (iorg = index of location in ptend_loc) 

! BEM Advected org is 3D: replace the 2D org with its value at steering level
! INITIAL GUESS use value at PBL top
! BEM later (inside iter_scaleh loop) will update it
! BEM grabbed this from below verbatim, it is to find kinv
       do k = mkx - 1, 1, -1 
...
99     continue    
       ...
       org(:iend)  = tr0_in(:iend,kinv,iorg)  !BEM 2010 set ORG to kinv (PBL top) value of ORG_3d
                                              !BEM 2010 will be overwritten with convecting layer avg below
       org_clipped = min(org, 1._r8)          !BEM clip for some uses

!       write(iulog,*) 'BEM org initial kinv ', kinv
!       write(iulog,*) 'BEM org, org_clipped at kinv ', org, org_clipped
!       write(iulog,*) 'BEM tr0_in at this time ', tr0_in(:iend,:,iorg)
!BEM org initial  -------------------------------------------------
...
! BEMens reinitialize the 14 key ensemble totals here inside iter_cin loop
cbmf_enstotal = 0.0_r8 ! BEMens 2010
umf_enstotal(:) = 0.0_r8 ! BEMens 2010
emf_enstotal(:) = 0.0_r8 ! BEMens 2010
slflx_enstotal(:) = 0.0_r8 ! BEMens 2010
...
! BEMens 2010: Loop over a plume ensemble
       kpenmax = 0  ! BEMens initialize kpenmax, ensemble top. (or could just use mkx)

       do iens = 1, NENSEMBLE    !BEMens 2010
!          write(iulog,*) 'BEM CIN iter, ensemble member ', iter, iens
       ! --------------------------------------------------------------------------- !
...
!          write(iulog,*) 'mu >= 3'
       ...
       cbmf = (rho0inv*sigmaw/2.5066_r8)*exp(-mu**2)    ! First cbmf attempt
...
!       write(iulog,*) 'BEM - closure done cbmf,winv', cbmf, winv

!----------------------------------------
!BEM ORGANIZED ENSEMBLE (of 2 members for the moment)
       rrkm = rkm  !BEMens default value (parameter rkm) 
!       write(iulog,*) 'BEM BEFORE org effects cbmf, rrkm, winv, thlsrc, qtsrc', cbmf, rrkm, winv, thlsrc, qtsrc

! ---------------------------------BEM ORG effect on whole ensemble (closure)
! BEMens Increase the net cloud base mass flux (relative to original closure)
! BEMens (this is an assumption of skewness of w, adjusting gaussian w assumption)

       cbmf = cbmf * ( 1.0_r8 + org(i)*org2cbmf1 )

!-----------------------------------BEMens ORG effects on higher ensemble members only
! ?? to clip or not to clip?? Only for probability interpretations.

       if( iens .gt. 1) then 
! BEMens Increase the cloud base mass flux for 2nd ensemble member by (1+org) factor
          cbmf = cbmf * (1.0_r8 + org(i)*org2cbmf2)
! BEMens Increase plume raduis type number by (1+org) factor
          rrkm = rkm /( 1.0_r8 + org(i)*org2rkm )  
! BEMens Increase plume w by org addition
          winv = winv + org2wpert*org(i)
! BEMens Increase plume Thl by org addition
          thlsrc = thlsrc + org2thpert*org(i)
! BEMens Increase plume qt by org addition
          qtsrc = qtsrc + org2qpert*org(i)
       endif
!-----------------------------------BEMens end ORG effects on higher ensemble members
!       write(iulog,*) 'BEM AFTER org effects cbmf, rrkm, winv, thlsrc, qtsrc', cbmf, rrkm, winv, thlsrc, qtsrc
!       write(iulog,*) 'BEM cloud base p, qt, thl', prel, qtsrc, thlsrc !BEMerr

!BEM Back to original  uwshcu code...

       ufrcinv = cbmf/winv/rho0inv 
!       write(iulog,*) 'BEM mass flux, ufrcinv', cbmf, ufrcinv !BEMerr
       ...
       if( ufrclcl .le. 0.0001_r8 ) then
           ...
           write(iulog,*) 'because cbmf, wlcl, rho0inv ', cbmf, wlcl, rho0inv
!           exit_ufrc(i) = 1._r8
!           id_exit = .true.
!           go to 333
       endif
       if( ufrclcl .le. 0.000001_r8 ) then
           write(iulog,*) 'SKIPPING: ufrclcl <= 0.000001' 
           exit_ufrc(i) = 1._r8
           id_exit = .true.
           go to 333
       endif
...
! BEM only do this if there's another go round to use it for
          if(iter_scaleh .lt. 3) then !BEM compute new org (from cumulus layer)

! BEM Here in iter_scaleh loop, update the layer mean org
! BEM Advected org is 3D: replace the org value here with its value at steering level
! BEM (or rather the integral over a steering layer) 
! BEM  so it advects in 2D with the steering level (or layer) flow

             if( iter_scaleh .eq. 1) kbup = 1
             orgsum(:iend) = 0._r8 
             do k = 1, kbup
                orgsum(:iend) = orgsum(:iend) + tr0_in(:iend,k,iorg)
             enddo
             org(:iend)  = orgsum(:iend)/kbup          !BEM 2010 set whole column to steering layer mean
             org_clipped = min(org, 1._r8)
          endif

!       write(iulog,*) 'BEM org iter_scaleh iteration:= ', iter_scaleh
!       write(iulog,*) 'BEM org layer is from k = 1 - ', kbup
!       write(iulog,*) 'BEM layer mean org, org_clipped: ', org, org_clipped
!       write(iulog,*) 'BEM profile of org (tr0_in): ', tr0_in(:iend,:,iorg)
...
!BEMens   rei(k) = ( 0.5_r8 * rkm / z0(k) / g /rho0j ) ! Alternative. -- from uwshcu.F90 original code
!BEMens I use rrkm instead of fixed parameter rkm 
          rei(k) = ( 0.5_r8 * rrkm / z0(k) / g /rho0j ) !BEMens variable rrkm (1/Pa) instead of parameter rkm 

!   Limiter for stability 
 ...
 45    continue
       ...
       end do ! end loop k = kpen, kbup calculating entrainment fluxes 
...
! BEMens 2010  qlten and qiten block: I needed to move the following inside the ensemble loop
! BEMens 2010  and this seemed like the place to put it. It was inside a k loop and is here too.
! BEMens 2010  AFTER DEBUG: I see I also need to be careful because
! BEMens 2010  dwten and diten (the dumped water) are used here with wrong units. The reason 
! BEMens 2010  is that umf is limited by a rmaxfrac before use in unit conversion, so the 
! BEMens 2010  unit conversion was put off in the original code. To solve it, move it below

! BEMens AFTER DEBUG I had to move the below dwten unit conversion up inside the iens loop here. 
          ! ------------------------------------------------------------------------- !     
          ...
          !! BEM the comment above is inconsisten with the code which uses umf(k-1) + umf(k)
          ! ------------------------------------------------------------------------- !

          dwten(k) = dwten(k) * 0.5_r8 * ( umf(k-1) + umf(k) ) * g / dp0(k) ! [ kg/kg/s ] BEMens
          diten(k) = diten(k) * 0.5_r8 * ( umf(k-1) + umf(k) ) * g / dp0(k) ! [ kg/kg/s ] BEMens
          ! dwten(k) = dwten(k) * umf(k) * g / dp0(k) ! [ kg/kg/s ]
          ! diten(k) = diten(k) * umf(k) * g / dp0(k) ! [ kg/kg/s ]


! BEMens I had to move the above dwten unit conversion up here, inside the iens loop. 
...
!BEMens this is the one 
!BEMens    logical , parameter              :: use_expconten = .true.
!BEMens    logical , parameter              :: use_unicondet = .false.
...
! BEMens  this rliq cumulative sum was snipped and brought into the iens loop here
          ! -------------------------------------------------------------------------- !
       ...
       end do !!! BEMens  end k = 1,kpen loop around moved qlten, qiten block





!BEMens moved the below block to here
       ! ---------------------------------------------------------------- !
...
! BEMens Note that all below is only for the last ensemble member now
          ...
          qlubelow = qlj
          qiubelow = qij
       end do ! k loop over levels krel to kpen

       ! ------------------------------------ !      
       ! Cloud top and base interface indices !
       ! ------------------------------------ !
       cnt = real( kpen, r8 )    !BEMens -- the last value set here will be output as cnt_out
       cnb = real( krel - 1, r8 )!BEMens -- the last value set here will be output as cnt_out

!BEMens moved the above block to here inside ensemble loop




! BEMens ------------------------------------------------------at end of ens loop
! BEMens 2010   Running total of 14 key variables for the ensemble of plumes

       if( kpen .gt. kpenmax ) kpenmax = kpen                ! BEMens 2010
       cbmf_enstotal = cbmf_enstotal + cbmf                ! BEMens 2010
       umf_enstotal(:mkx) = umf_enstotal(:mkx) + umf(:mkx) ! BEMens 2010
       emf_enstotal(:mkx) = emf_enstotal(:mkx) + emf(:mkx) ! BEMens 2010
       slflx_enstotal(:mkx) = slflx_enstotal(:mkx) + slflx(:mkx) ! BEMens 2010
       qtflx_enstotal(:mkx) = qtflx_enstotal(:mkx) + qtflx(:mkx) ! BEMens 2010
       uflx_enstotal(:mkx) = uflx_enstotal(:mkx) + uflx(:mkx) ! BEMens 2010
       vflx_enstotal(:mkx) = vflx_enstotal(:mkx) + vflx(:mkx) ! BEMens 2010
       trflx_enstotal(:mkx,:ncnst) = trflx_enstotal(:mkx,:ncnst) + trflx(:mkx,:ncnst) ! BEMens 2010
       dwten_enstotal(:mkx) = dwten_enstotal(:mkx) + dwten(:mkx) ! BEMens 2010
       diten_enstotal(:mkx) = diten_enstotal(:mkx) + diten(:mkx) ! BEMens 2010
       qlten_enstotal(:mkx) = qlten_enstotal(:mkx) + qlten(:mkx) ! BEMens 2010
       qiten_enstotal(:mkx) = qiten_enstotal(:mkx) + qiten(:mkx) ! BEMens 2010
       qc_enstotal(:mkx) = qc_enstotal(:mkx) + qc(:mkx) ! BEMens 2010
       qc_l_enstotal(:mkx) = qc_l_enstotal(:mkx) + qc_l(:mkx) ! BEMens 2010
       qc_i_enstotal(:mkx) = qc_i_enstotal(:mkx) + qc_i(:mkx) ! BEMens 2010
       ufrc_enstotal(:mkx) = ufrc_enstotal(:mkx) + ufrc(:mkx) ! BEMens 2010
       cufrc_enstotal(:mkx) = cufrc_enstotal(:mkx) + cufrc(:mkx) ! BEMens 2010
!       write(iulog,*) 'BEMbug at summing time iens,ufrc,cufrc_enstotal', iens, ufrc(kbup), cufrc_enstotal(kbup)
       qcu_enssum(:mkx) = qcu_enssum(:mkx) + qcu(:mkx)*cufrc(:mkx) ! BEMens 2010
       qlu_enssum(:mkx) = qlu_enssum(:mkx) + qlu(:mkx)*cufrc(:mkx) ! BEMens 2010
       qiu_enssum(:mkx) = qiu_enssum(:mkx) + qiu(:mkx)*cufrc(:mkx) ! BEMens 2010

!BEM individual plume fields
!    if (iens .eq. 1)  plume1_b(i,0) = 0._r8                    !BEM zero at surface
!    if (iens .eq. 1)  plume1_b(i,:mkx) = bogtop_arr            !BEM buoyancy at top interfaces
    if (iens .eq. 1)  plume1_w(i,0:mkx) = wu(0:mkx)             !BEM updraft vertical velocity
    if (iens .eq. 1)  plume1_Mup(i,0:mkx) = umf(0:mkx) 
    if (iens .eq. 1)  plume1_MF(i,0:mkx) = umf(0:mkx)  + emf(0:mkx) 
    if (iens .eq. 1)  plume1_ufrc(i,0:mkx) = ufrc(0:mkx) 

!    if (iens .ne. 0)  plumeN_b(i,:mkx) = bogtop_arr            !BEM buoyancy at top interfaces
    if (iens .ne. 0)  plumeN_w(i,0:mkx) = wu(0:mkx)             !BEM updraft vertical velocity
    if (iens .ne. 0)  plumeN_Mup(i,0:mkx) = umf(0:mkx) 
    if (iens .ne. 0)  plumeN_MF(i,0:mkx) = umf(0:mkx)  + emf(0:mkx) 
    if (iens .ne. 0)  plumeN_ufrc(i,0:mkx) = ufrc(0:mkx) 

! BEMens 2010: Close the NENSEMBLE loop
       end do !BEMens end of iens = 1, NENSEMBLE loop

! BEMens 2010   Rename the 14 _total variables from the ensemble total to their original 
! BEMens 2010   names, so the rest of the code (tendencies etc.) is unchanged from uwshcu.F90. 
! BEMens 2010   Variables besides these 14 (diag. outputs etc.) will refer to the last member. 

       cbmf = cbmf_enstotal           ! BEMens 2010
       umf(:mkx) = umf_enstotal(:mkx) ! BEMens 2010
       emf(:mkx) = emf_enstotal(:mkx) ! BEMens 2010
       slflx(:mkx) = slflx_enstotal(:mkx) ! BEMens 2010
       qtflx(:mkx) = qtflx_enstotal(:mkx) ! BEMens 2010
       uflx(:mkx) = uflx_enstotal(:mkx) ! BEMens 2010
       vflx(:mkx) = vflx_enstotal(:mkx) ! BEMens 2010
       trflx(:mkx,:ncnst) = trflx_enstotal(:mkx,:ncnst) ! BEMens 2010
       dwten(:mkx) = dwten_enstotal(:mkx) ! BEMens 2010
       diten(:mkx) = diten_enstotal(:mkx) ! BEMens 2010
       qlten(:mkx) = qlten_enstotal(:mkx) ! BEMens 2010
       qiten(:mkx) = qiten_enstotal(:mkx) ! BEMens 2010
       qc(:mkx) = qc_enstotal(:mkx) ! BEMens 2010
       qc_l(:mkx) = qc_l_enstotal(:mkx) ! BEMens 2010
       qc_i(:mkx) = qc_i_enstotal(:mkx) ! BEMens 2010
       cufrc(:mkx) = cufrc_enstotal(:mkx) ! BEMens 2010
       ufrc(:mkx) = ufrc_enstotal(:mkx) ! BEMens 2010
!       write(iulog,*) 'BEMbug after consolidation cufrc,cufrc_enstotal', cufrc(kbup), cufrc_enstotal(kbup)
       qcu(:mkx) = qcu_enssum(:mkx)/max(cufrc(:mkx),1.e-15_r8)  ! BEMens 2010 area weighted average
       qlu(:mkx) = qlu_enssum(:mkx)/max(cufrc(:mkx),1.e-15_r8)  ! BEMens 2010 cu area weighted avg
       qiu(:mkx) = qiu_enssum(:mkx)/max(cufrc(:mkx),1.e-15_r8)  ! BEMens 2010 cufrc = 2*ufrc recall
! BEMens --------------------------------------------------------- end ens loop       

! BEMens 2010 Now back to the code as I found it, except I moved a 
! BEMens 2010 qlten and qiten block inside the ensemble loop, and 
! BEMens 2010 replace kpen with kpenmax in some loops below
...
!BEMens       rliq    = 0._r8       !why was this here? kill it
       rainflx = 0._r8
       snowflx = 0._r8
...
! BEMens I had to move the below dwten unit conversion up into the iens loop. 
! BEMens For this, I also had to move the rmaxfrac constraint up into the iens loop. 
          ! ------------------------------------------------------------------------- !     
          ! 'dwten(k)','diten(k)' : Production rate of condensate  within the layer k !
          !      [ kg/kg/s ]        by the expels of condensate from cumulus updraft.... !
          ! ... more cut and moved -------------------------------------------------- !

!BEMens          dwten(k) = dwten(k) * 0.5_r8 * ( umf(k-1) + umf(k) ) * g / dp0(k) ! [ kg/kg/s ]
!BEMens          diten(k) = diten(k) * 0.5_r8 * ( umf(k-1) + umf(k) ) * g / dp0(k) ! [ kg/kg/s ]  
! BEMens I had to move the above dwten unit conversion up into the iens loop. 
...
! BEMens -- I took a small 2-line block from here up into the iens loop
! BEMens this moved up into iens loop              qc(k)  =  qc_l(k) +  qc_i(k)   
! BEMens this was moved up into iens loop          rliq   =  rliq    + qc(k) * dp0(k) / g / 1000._r8    ! [ m/s ]

          qvten(k) = qtten(k) - qlten(k) - qiten(k)
          sten(k)  = slten(k) + xlv * qlten(k) + xls * qiten(k)

       end do ! k loop up to kpenmax BEMens

       precip  =  rainflx + snowflx                       ! [ kg/m2/s ]
       snow    =  snowflx                                 ! [ kg/m2/s ] 

!    write(iulog,*) 'BEMens kpen, kbup', kpen, kbup
!    write(iulog,*) 'BEMens precip1, snow1 ', precip, snow
...
!    write(iulog,*) 'BEMens precip2, snow2 ', precip, snow
!BEMerr write out the tendency profiles
!    write(iulog,*) 'BEMens slflx (K/d)', slflx !BEMerr
!    write(iulog,*) 'BEMens qtflx (K/d)', qtflx !BEMerr
!    write(iulog,*) 'BEMens slten (K/d)', slten  *86400._r8 !BEMerr
!    write(iulog,*) 'BEMens xlv, xls ', xlv, xls            !BEMerr 
!    write(iulog,*) 'BEMens sten (K/d)', sten    *86400._r8 !BEMerr
!    write(iulog,*) 'BEMens qtten (g/kg/d)', qtten *864._r8 !BEMerr
!    write(iulog,*) 'BEMens qlten (g/kg/d)', qlten *864._r8 !BEMerr
!    write(iulog,*) 'BEMens qiten (g/kg/d)', qiten *864._r8 !BEMerr
!    write(iulog,*) 'BEMens qvten (g/kg/d)', qvten *864._r8 !BEMerr
!    write(iulog,*) 'BEMens last print stmt of uwshcu **********************'
...
!       do k = 1, kpen
       ...
       enddo ! closing  m = 4,ncnst loop

!BEMens moved a block from here up into iens ensemble loop, find it above
       ! ---------------------------------------------------------------- !
       ! Cumpute default diagnostic outputs                               !
!.....
       ! ------------------------------------------------------------------------- !
...
!the indices are not reversed, these variables go into compute_mcshallow_inv, this is why they are called "flxprc1" and "flxsnow1".
     ...
     qrten_out(i,:mkx)            = qrten(:mkx) !BEM cannot redefine as liq+ice since it is a true output (passed out of scheme)
     ...
     dwten_out(i,mkx:1:-1)        = dwten(:mkx) + diten(:mkx) !BEM redefined this as ice+liquid
     diten_out(i,mkx:1:-1)        = diten(:mkx)
     flxrain_out(i,mkx:0:-1)      = flxrain(0:mkx) + flxsnow(0:mkx) !BEM redefined this as ice+liquid
     qpten_out(i,mkx:0:-1)        = qrten(0:mkx) + qsten(0:mkx) !BEM redefined this as ice+liquid
     qcten_out(i,mkx:0:-1)        = qlten(0:mkx) + qiten(0:mkx) !BEM redefined this as ice+liquid

     flxsnow_out(i,mkx:0:-1)      = flxsnow(0:mkx)
     ntraprd_out(i,mkx:1:-1)      = ntraprd(:mkx) + ntsnprd(:mkx) !BEM redefined this as ice+liquid
...
!------------------------------------------ BEM after rain in final iteration 
! BEM for each column (i), solve the org2D evolution equation
! BEM d(org2D)/dt = -org2D/tauORG + source 
! BEM In order to set org3d at all levels to this desired org2d value, need to 
! BEM build a tracer tendency to pass out to physics_update equal to 
! BEM (-org3d + org2d) / dt
! BEM This tendency is to be carried via ptend_loc out to convect_shallow.F90

! BEM 2010 for org source: (evpint_rain and evap_snow) are in mm/s (kg/m2/s) units
! BEM There was a lot of org from high level snow evap, so make this rain only
! BEM for this overwrite let's just do the exponential decay then add source 
! BEM rather than time stepping an ODE somehow

!     write(iulog,*) 'BEM org value before update ', org(i)
!     write(iulog,*) 'BEM value of dt, dt/tauORG ', dt, dt/tauORG

     org(i) = org(i)*(1-(dt/tauORG)) 

!     write(iulog,*) 'BEM org value after decay term ', org(i)

     org(i) = org(i) + evap2org*(evpint_rain)*dt 

! Add some coastal organization BEM 2011 coast org
    
      call get_curr_date(yr,mon,day,tod) 
      GMT    = real(tod)/3600.0
      LT(i)   = GMT+ (((lon(i)*180._r8)/3.1415926)/15.0)           !  GMT trans into local time 
      if(LT(i).gt.24) then
        LT(i)=LT(i)-24
      endif

      if( landfrac(i) .gt. 0.1_r8 .and. landfrac(i) .lt. 0.9_r8 ) then 
         org(i) = org(i) + coast2org*(cos(2._r8*3.1415926/24._r8*(LT(i)+6._r8))**2._r8)*4._r8*dt
     endif
      if(masterproc) then 
        write(iulog,*) 'Longitude=', lon(i)
        write(iulog,*) 'GMT=',GMT
        write(iulog,*) 'Local time=', LT(i)
      endif
	
!     write(iulog,*) 'BEM org value after source term ', org(i)
!     write(iulog,*) 'BEM org source term (x dt) ', evap2org*(evpint_rain)*dt

! BEM Now 'overwrite' the org3D at every level by crating a tendency that 
! BEM destroys what is there and adds the number we want

     do k = 1, mkx 
        trten_out(i,k,iorg) = (-tr0_in(i,k,iorg) + org(i)) /dt
     enddo

!     write(iulog,*) 'BEM rain evaporation: ', evpint_rain
!     write(iulog,*) 'BEM org3d tendency ', (-tr0_in(i,1,iorg) + org(i)) /dt
!     write(iulog,*) 'BEM org3d tendency x dt ', (-tr0_in(i,1,iorg)/dt + org(i)) 


! BEM output the source term for org, evaporation of precip
     evapprec_out(i) = evpint_rain+evpint_snow
     evaprain_out(i) = evpint_rain
!------------------------------------------ BEM end of org update block
 ...
 333 if(id_exit) then ! Exit without cumulus convection
     ...
     qpten_out(i,mkx:1:-1)       = 0._r8    
     qcten_out(i,mkx:1:-1)       = 0._r8    
...
! BEM ---------------------------------- BEM second org update block
        call cnst_get_ind('ORG_ADV',iorg)       !BEM 2010 (iorg = index of location in ptend_loc) 
        org(i)  = tr0_in(i,1,iorg)  !BEM 2010 set ORG to kinv (PBL top) value of ORG_3d
        org(i) = org(i)*exp(-dt/tauORG) 
        
        call get_curr_date(yr,mon,day,tod)
        GMT    = real(tod)/3600._r8       
        LT(i)   = GMT+ (((lon(i)*180._r8)/3.1415926)/15.0)           !  GMT trans into local time 
        if(LT(i).gt.24) then
            LT(i)=LT(i)-24
        endif
...
! BEM----------------------------------
     ...
     end if !!!! this was the 333 case of no cumulus convection
...
!     write(iulog,*) 'BEM calling outfld'
     call outfld( 'Cu_evaprain'     , evaprain_out,             mix,    lchnk ) 
     call outfld( 'Cu_evapprec'     , evapprec_out,             mix,    lchnk ) 

     call outfld( 'Cu_qtflx'        , qtflx_out(:,mkx:0:-1),    mix,    lchnk ) 
     call outfld( 'Cu_slflx'        , slflx_out(:,mkx:0:-1),    mix,    lchnk ) 
     call outfld( 'Cu_uflx'         , uflx_out,                 mix,    lchnk ) 
     call outfld( 'Cu_vflx'         , vflx_out,                 mix,    lchnk ) 

     call outfld( 'Cu_qtten'        , qtten_out,                mix,    lchnk ) 
     call outfld( 'Cu_slten'        , slten_out,                mix,    lchnk ) 
     call outfld( 'Cu_uten'         , uten_out(:,mkx:1:-1),     mix,    lchnk ) 
     call outfld( 'Cu_vten'         , vten_out(:,mkx:1:-1),     mix,    lchnk ) 
     call outfld( 'Cu_qvten'        , qvten_out(:,mkx:1:-1),    mix,    lchnk ) 
     call outfld( 'Cu_qlten'        , qlten_out(:,mkx:1:-1),    mix,    lchnk )
     call outfld( 'Cu_qiten'        , qiten_out(:,mkx:1:-1),    mix,    lchnk )   
     call outfld( 'Cu_qcten'        , qcten_out(:,mkx:1:-1),    mix,    lchnk )   

     call outfld( 'Cu_cbmf'         , cbmf_out,                 mix,    lchnk ) 
     call outfld( 'Cu_ufrcinvbase'  , ufrcinvbase_out,          mix,    lchnk ) 
     call outfld( 'Cu_ufrclcl'      , ufrclcl_out,              mix,    lchnk ) 
     call outfld( 'Cu_winvbase'     , winvbase_out,             mix,    lchnk ) 
     call outfld( 'Cu_wlcl'         , wlcl_out,                 mix,    lchnk ) 
     call outfld( 'Cu_plcl'         , plcl_out,                 mix,    lchnk ) 
     call outfld( 'Cu_pinv'         , pinv_out,                 mix,    lchnk ) 
     call outfld( 'Cu_plfc'         , plfc_out,                 mix,    lchnk ) 
     call outfld( 'Cu_pbup'         , pbup_out,                 mix,    lchnk ) 
     call outfld( 'Cu_ppen'         , ppen_out,                 mix,    lchnk ) 
     call outfld( 'Cu_srcqt'        , qtsrc_out,                mix,    lchnk ) 
     call outfld( 'Cu_srcthl'       , thlsrc_out,               mix,    lchnk ) 
     call outfld( 'Cu_srcthvl'      , thvlsrc_out,              mix,    lchnk ) 
     call outfld( 'Cu_emfkbup'      , emfkbup_out,              mix,    lchnk )
     call outfld( 'Cu_cin'          , cinh_out,                 mix,    lchnk )  
     call outfld( 'Cu_cinlcl'       , cinlclh_out,              mix,    lchnk ) 
     call outfld( 'Cu_cbmflimit'    , cbmflimit_out,            mix,    lchnk ) 
     call outfld( 'Cu_tkeavg'       , tkeavg_out,               mix,    lchnk )
     call outfld( 'Cu_zinv'         , zinv_out,                 mix,    lchnk )  
     call outfld( 'Cu_rcwp'         , rcwp_out,                 mix,    lchnk )
     call outfld( 'Cu_rlwp'         , rlwp_out,                 mix,    lchnk )
     call outfld( 'Cu_riwp'         , riwp_out,                 mix,    lchnk )
     call outfld( 'Cu_tophgt'       , cush_inout,               mix,    lchnk )   

     call outfld( 'Cu_wu'           , wu_out,                   mix,    lchnk )
     call outfld( 'Cu_ufrc'         , ufrc_out,                 mix,    lchnk )
     call outfld( 'Cu_qtu'          , qtu_out,                  mix,    lchnk )
     call outfld( 'Cu_thlu'         , thlu_out,                 mix,    lchnk )
     call outfld( 'Cu_thvu'         , thvu_out,                 mix,    lchnk )
     call outfld( 'Cu_uu'           , uu_out,                   mix,    lchnk )
     call outfld( 'Cu_vu'           , vu_out,                   mix,    lchnk )
!     call outfld( 'Cu_qtu_emf'      , qtu_emf_out,              mix,    lchnk )
!    call outfld( 'Cu_thlu_emf'     , thlu_emf_out,             mix,    lchnk )
!     call outfld( 'Cu_uu_emf'       , uu_emf_out,               mix,    lchnk )
!     call outfld( 'Cu_vu_emf'       , vu_emf_out,               mix,    lchnk )
     call outfld( 'Cu_umf'          , umf_out(:,mkx:0:-1),      mix,    lchnk )
     call outfld( 'Cu_uemf'         , uemf_out,                 mix,    lchnk )

     call outfld( 'Cu_qcu'          , qcu_out(:,mkx:1:-1),      mix,    lchnk )
     call outfld( 'Cu_qlu'          , qlu_out(:,mkx:1:-1),      mix,    lchnk )
     call outfld( 'Cu_qiu'          , qiu_out(:,mkx:1:-1),      mix,    lchnk )
     call outfld( 'Cu_cufrc'        , cufrc_out(:,mkx:1:-1),    mix,    lchnk )  
     call outfld( 'Cu_fer'          , fer_out,                  mix,    lchnk )  
     call outfld( 'Cu_fdr'          , fdr_out,                  mix,    lchnk )  

!!! BEM Output org
!     write(iulog,*) 'BEM calling outfld for ORG'
     call outfld('ORG'               ,org,              mix,    lchnk) !BEM

!BEM horizontal divergence inflow/outflow type output block
!     vergin(:,1:mkx) = fer_out(:,1:mkx)*umf_out(:,0:mkx-1)    *9.8_r8/dp0
!     vergout(:,1:mkx) = fdr_out(:,1:mkx)*umf_out(:,0:mkx-1)   *9.8_r8/dp0
!     vergence(:,1:mkx) = (fdr_out(:,1:mkx)-fer_out(:,1:mkx))*umf_out(:,0:mkx-1) *9.8_r8/dp0
!     dMdp(:,1:mkx) = (uemf_out(:,1:mkx) - uemf_out(:,0:mkx-1)) *9.8_r8/dp0
!     call outfld( 'Cu_vergin'        , vergin,                  mix,    lchnk )
!     call outfld( 'Cu_vergout'       , vergout,                 mix,    lchnk )
!     call outfld( 'Cu_vergence'      , vergence,                mix,    lchnk )
!     call outfld( 'Cu_dMdp'          , dMdp,                    mix,    lchnk )

!BEM vergence output block

!BEM renamed these for alphabetical order and clarity and added revap
!     write(iulog,*) 'BEM calling outfld for pre/pri variables'
     call outfld( 'Cu_preover'       , dwten_out,                mix,    lchnk )
     call outfld( 'Cu_priover'       , diten_out,                mix,    lchnk )
!     call outfld( 'Cu_preprod'       , qpten_out(:,mkx:1:-1),    mix,    lchnk ) !BEM why invert??
!     call outfld( 'Cu_priprod'       , qsten_out(:,mkx:1:-1),    mix,    lchnk ) !BEM why invert??
     call outfld( 'Cu_preprod'       , qpten_out,                mix,    lchnk )
     call outfld( 'Cu_priprod'       , qsten_out,                mix,    lchnk )
     call outfld( 'Cu_preprodnet'    , ntraprd_out,              mix,    lchnk )
     call outfld( 'Cu_priprodnet'    , ntsnprd_out,              mix,    lchnk )
     call outfld( 'Cu_preqflx'       , flxrain_out,              mix,    lchnk )
     call outfld( 'Cu_priqflx'       , flxsnow_out,              mix,    lchnk )
!     call outfld( 'Cu_prerevap'      , evapc_out(:,mkx:1:-1),    mix,    lchnk )

!BEM  convert to heating rate units? 
!     evapc_out(i,mkx:1:-1) = evapc_out(i,mkx:1:-1) * 2.5e6_r8 /1004._r8 /(dp0(mkx:1:-1)/9.8_r8) !BEM unit conversion to K/s
!     call outfld( 'Cu_prerevapcool'  , evapc_out,                mix,    lchnk ) evapc_out is a real output, don't overwrite
!BEM end precip output block

!BEM individual plume fields, they are upside down so flip em
!     call outfld( 'Cu_plume1_b'         , plume1_b(:,mkx:0:-1),                 mix,    lchnk )
!     write(iulog,*) 'BEM calling outfld for plume1_* variables'
     call outfld( 'Cu_plume1_w'         , plume1_w(:,mkx:0:-1),                 mix,    lchnk )
     call outfld( 'Cu_plume1_Mup'       , plume1_Mup(:,mkx:0:-1),               mix,    lchnk )
     call outfld( 'Cu_plume1_MF'        , plume1_MF(:,mkx:0:-1),                mix,    lchnk )
     call outfld( 'Cu_plume1_ufrc'      , plume1_ufrc(:,mkx:0:-1),              mix,    lchnk )

!     call outfld( 'Cu_plumeN_b'         , plumeN_b(:,mkx:0:-1),                 mix,    lchnk )
     call outfld( 'Cu_plumeN_w'         , plumeN_w(:,mkx:0:-1),                 mix,    lchnk )
     call outfld( 'Cu_plumeN_Mup'       , plumeN_Mup(:,mkx:0:-1),               mix,    lchnk )
     call outfld( 'Cu_plumeN_MF'        , plumeN_MF(:,mkx:0:-1),                mix,    lchnk )
     call outfld( 'Cu_plumeN_ufrc'      , plumeN_ufrc(:,mkx:0:-1),              mix,    lchnk )

!BEM end 
     call outfld( 'Cu_excessu'      , excessu_arr_out,          mix,    lchnk )
!     call outfld( 'Cu_excess0'      , excess0_arr_out,          mix,    lchnk )
     call outfld( 'Cu_xc'           , xc_arr_out,               mix,    lchnk )
     call outfld( 'Cu_aquad'        , aquad_arr_out,            mix,    lchnk )
     call outfld( 'Cu_bquad'        , bquad_arr_out,            mix,    lchnk )
     call outfld( 'Cu_cquad'        , cquad_arr_out,            mix,    lchnk )
     call outfld( 'Cu_bogbot'       , bogbot_arr_out,           mix,    lchnk )
     call outfld( 'Cu_bogtop'       , bogtop_arr_out,           mix,    lchnk )

     call outfld( 'Cu_exit_UWCu'    , exit_UWCu,                mix,    lchnk ) 
     call outfld( 'Cu_exit_conden'  , exit_conden,              mix,    lchnk ) 
     call outfld( 'Cu_exit_klclmkx' , exit_klclmkx,             mix,    lchnk ) 
     call outfld( 'Cu_exit_klfcmkx' , exit_klfcmkx,             mix,    lchnk ) 
     call outfld( 'Cu_exit_ufrc'    , exit_ufrc,                mix,    lchnk ) 
     call outfld( 'Cu_exit_wtw'     , exit_wtw,                 mix,    lchnk ) 
     call outfld( 'Cu_exit_drycore' , exit_drycore,             mix,    lchnk ) 
     call outfld( 'Cu_exit_wu'      , exit_wu,                  mix,    lchnk ) 
     call outfld( 'Cu_exit_cufilter', exit_cufilter,            mix,    lchnk ) 
     call outfld( 'Cu_exit_kinv1'   , exit_kinv1,               mix,    lchnk ) 
     call outfld( 'Cu_exit_rei'     , exit_rei,                 mix,    lchnk ) 

     call outfld( 'Cu_limit_shcu'   , limit_shcu,               mix,    lchnk ) 
     call outfld( 'Cu_limit_negcon' , limit_negcon,             mix,    lchnk ) 
     call outfld( 'Cu_limit_ufrc'   , limit_ufrc,               mix,    lchnk ) 
     call outfld( 'Cu_limit_ppen'   , limit_ppen,               mix,    lchnk ) 
     call outfld( 'Cu_limit_emf'    , limit_emf,                mix,    lchnk ) 
     call outfld( 'Cu_limit_cinlcl' , limit_cinlcl,             mix,    lchnk ) 
     call outfld( 'Cu_limit_cin'    , limit_cin,                mix,    lchnk ) 
     call outfld( 'Cu_limit_cbmf'   , limit_cbmf,               mix,    lchnk ) 
     call outfld( 'Cu_limit_rei'    , limit_rei,                mix,    lchnk ) 
     call outfld( 'Cu_ind_delcin'   , ind_delcin,               mix,    lchnk ) 

      if (masterproc) then
!         write(iulog,*) "hyma uwshcu.F90 1"
      endif

      if (masterproc) then
!         write(iulog,*) "hyma uwshcu.F90 2"
      endif

!BEMens 2010 test if outputs are all same as control
!write(iulog,*) 'BEMens umf_out, slflx_out, qtflx_out, qvten_out, qlten_out, qiten_out, sten_out, uten_out', &
!umf_out(kbup), slflx_out(kbup), qtflx_out(kbup), qvten_out(kbup), qlten_out(kbup), &
!qiten_out(kbup), sten_out(kbup), uten_out(kbup)
!write(iulog,*) 'BEMens vten_out, qrten_out, qsten_out, precip_out, snow_out, evapc_out, cufrc_out', &
!vten_out(kbup), qrten_out(kbup), qsten_out(kbup), precip_out, &
!snow_out, evapc_out(kbup), cufrc_out(kbup)
!write(iulog,*) 'BEMens qcu_out, qlu_out, qiu_out, cbmf_out, qc_out, rliq_out, cnt_out, cnb_out', &
!qcu_out(kbup), qlu_out(kbup), qiu_out(kbup), cbmf_out, qc_out(kbup), &
!rliq_out, cnt_out, cnb_out
  ...
  ! ------------------------------------------------------------------------------- !
    ...
    if( dqv .gt. 1.e-20_r8 ) then
        ...
        if( aa .lt. 0.5_r8 ) then
            ...
            write(6,*) 'Full positive_moisture is impossible in uwshcu'
        endif
    endif 
