  ...
  use constituents, only: cnst_add    !BEM 2010
  use physconst,   only: mwdry, cpair !BEM dry air molecular weight and specific heat
  implicit none
  integer idx
  integer org_cnst_ind !BEMorg
  ...
  call pbuf_add('SH_CLDICE', 'physpkg', 1, pver,  1, sh_cldice_idx)  ! shallow gbm cloud ice water (kg/kg)
    ...
    call cnst_add('ORG_ADV', mwdry, cpair, 0._r8, org_cnst_ind, &   !BEM 2010
         longname = 'Convective organization',readiv=.false.,mixtype = 'dry')  !BEM 2010
  ...
  subroutine convect_shallow_tend( ztodt  , qpert   , pblht    , &
                                   cmfmc  , cmfmc2  , precc    , &
                                   qc     , qc2     , rliq     , rliq2    , & 
                                   snow   , state   , ptend_all, landfrac, pbuf )
   ...
   real(r8),            intent(in)    :: landfrac(pcols)          ! Land fraction BEM 2011
   ...
   case('UW')   ! UW shallow convection scheme
      ...
      !      write(6,*) 'Sungsu : convect_shallow.F90 does not conserve tracers : ', m, sum1, sum2, abs(sum2-sum1)/sum1
      !!     write(6,*) 'Sungsu : convect_shallow.F90 does not conserve tracers : ', m, sum3
      !  endif
      !  enddo
      !  enddo
